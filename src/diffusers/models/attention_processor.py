...
class FluxAttnProcessor2_0:
    """Attention processor using Flash Attention with optimized dimension ordering."""
    def __init__(self):
        try:
            from flash_attn import flash_attn_func
            self.flash_attn_func = flash_attn_func
        except ImportError:
            raise ImportError("flash-attn is not installed. Please build from source.")
    ...
    def __call__(
        ...
        # Reshape directly to flash attention format (batch, seq_len, heads, head_dim)
        query = query.view(batch_size, -1, attn.heads, head_dim)
        key = key.view(batch_size, -1, attn.heads, head_dim)
        value = value.view(batch_size, -1, attn.heads, head_dim)
        ...
        hidden_states = self.flash_attn_func(
            q=query,
            k=key,
            v=value,
            causal=False,
        ...
        # Reshape to (batch_size, seq_len, heads * head_dim)
